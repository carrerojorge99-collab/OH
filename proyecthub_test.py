...
class ProyectHubTester:
    ...
    def test_create_required_documents(self):
        ...
        try:
            ...
            if response.status_code == 200:
                self.log_test("Required Documents - Create From Client", True, 
                            f"Created document: {from_client_data['title']}")
            else:
                try:
                    error_data = response.json()
                    error_msg = error_data.get('detail', f"HTTP {response.status_code}")
        ...
        to_client_data = {
            "title": f"Test Document to Client - {datetime.now().strftime('%Y-%m-%d %H:%M')}",
            "description": "This is a test document that should be sent to the client",
            "required": True,
            "completed": False
        }
