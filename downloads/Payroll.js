...
const Payroll = () => {
  ...
  const savePayroll = async () => {
    ...
    try {
      ...
      const response = await api.post(`/payroll/process`, payload, { withCredentials: true });
      
      // Show success message with accounting sync status
      if (response.data.accounting_synced) {
        toast.success('Nómina guardada y registrada en contabilidad', {
          description: `Entrada de diario #${response.data.journal_entry_id?.substring(0, 8) || ''} creada automáticamente`
        });
      } else {
        toast.success('Nómina guardada exitosamente');
        if (response.data.accounting_message) {
          toast.info('Nota: ' + response.data.accounting_message, {
            description: 'Puede sincronizar manualmente desde el módulo de Contabilidad'
          });
        }
      }
