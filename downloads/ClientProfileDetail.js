...
const ClientProfileDetail = () => {
  ...
  const openEditEstimate = (estimate) => {
    setEstimateForm({
      ...
      document_date: estimate.document_date || moment(estimate.created_at).format('YYYY-MM-DD')
    });
  ...
  const handleEstimateSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        document_date: estimateForm.document_date
      };
  ...
  const exportPDF = async (estimate) => {
    try {
      ...
      // Usar document_date si existe, sino created_at
      const documentDate = estimate.document_date || estimate.created_at;
      let y = await addDocumentHeader(doc, company, 'ESTIMATE', estimate.estimate_number, documentDate, estimate.total);
      ...
      // Notes and Terms - Check if space available, only add page if needed
      ...
      if (estimate.notes || estimate.terms) {
        ...
        doc.setFontSize(8);
        const termsLines = estimate.terms ? doc.splitTextToSize(estimate.terms, columnWidth) : [];
        const notesLines = estimate.notes ? doc.splitTextToSize(estimate.notes, columnWidth) : [];
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-slate-400 flex items-center gap-1">
                                ...
                                {moment(estimate.document_date || estimate.created_at).format('DD/MM/YYYY')}
                              </p>
                            </div>
                          </div>
                          <div className="flex items-center gap-4">
            ...
            <form onSubmit={handleEstimateSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <Input type="date" value={estimateForm.document_date} onChange={(e) => setEstimateForm({...estimateForm, document_date: e.target.value})} />
                </div>
                <div>
                  <Label>Preparado por *</Label>
                  <Input 
                    value={estimateForm.prepared_by} 
                    onChange={(e) => setEstimateForm({...estimateForm, prepared_by: e.target.value})} 
            ...
            {previewEstimate && (
              <div className="space-y-6 p-4 bg-white border rounded-lg">
                ...
                <div className="flex justify-between items-start border-b pb-4">
                  ...
                  <div className="text-right">
                    <p className="text-sm text-slate-500">Fecha: {moment(previewEstimate.document_date || previewEstimate.created_at).format('DD/MM/YYYY')}</p>
