...
import { 
  ...
  ArrowLeft, Calculator, Users, AlertTriangle, UserPlus
} from 'lucide-react';
...
const CostEstimatesByCompany = () => {
  ...
  // Get estimates WITHOUT sponsor for a company (orphaned estimates)
  const getOrphanedEstimates = (companyId) => {
    return estimates.filter(e => e.company_id === companyId && !e.sponsor_id);
  };
  ...
  const handleAssignEstimate = async () => {
    ...
    // Get sponsors for the selected company
    const selectedComp = companies.find(c => c.company_id === assignForm.company_id);
    const sponsors = selectedComp?.sponsors || [];
    
    // Require sponsor if company has sponsors
    if (sponsors.length > 0 && !assignForm.sponsor_id) {
      toast.error('Debe seleccionar un sponsor');
      return;
    }
    
    // If company has no sponsors, show error
    if (sponsors.length === 0) {
      toast.error('Esta compañía no tiene sponsors. Cree uno primero.');
      return;
    }
    
    try {
      await api.put(`/cost-estimates/${estimateToAssign.estimate_id || estimateToAssign.cost_estimate_id}/assign`, {
              ...
              {sponsorEstimates.length === 0 ? (
                ...
                <div className="space-y-3">
                  {sponsorEstimates.map(estimate => (
                    <div 
                      key={estimate.estimate_id || estimate.cost_estimate_id}
                          ...
                          {showMoney && (
                            <>
                              <Button 
                                variant="ghost" 
                                size="sm"
                                onClick={() => window.open(`${getBackendUrl()}/api/cost-estimates/${estimate.estimate_id || estimate.cost_estimate_id}/pdf`, '_blank')}
                              >
                              ...
                              <Button 
                                variant="ghost" 
                                size="sm"
                                onClick={() => handleDeleteEstimate(estimate.estimate_id || estimate.cost_estimate_id)}
                              >
    ...
    return (
      <Layout>
        <div className="space-y-6">
          ...
          {/* Orphaned Estimates (without sponsor) */}
          {getOrphanedEstimates(selectedCompany.company_id).length > 0 && (
            <Card className="border-amber-300 bg-amber-50">
              <CardContent className="p-6">
                <h3 className="text-lg font-semibold mb-4 flex items-center gap-2 text-amber-700">
                  <AlertTriangle className="w-5 h-5" />
                  Estimaciones Sin Sponsor Asignado
                </h3>
                <p className="text-sm text-amber-600 mb-4">
                  Estas estimaciones están asignadas a la compañía pero no tienen un sponsor. Haz clic en "Asignar Sponsor" para vincularlas.
                </p>
                <div className="space-y-3">
                  {getOrphanedEstimates(selectedCompany.company_id).map(estimate => (
                    <div 
                      key={estimate.estimate_id}
                      className="flex items-center justify-between p-4 bg-white rounded-lg border border-amber-200"
                    >
                      <div>
                        <p className="font-semibold">{estimate.estimate_name || estimate.estimate_number || 'Sin nombre'}</p>
                        <p className="text-sm text-slate-500">
                          {estimate.estimate_number} • Total: ${(estimate.grand_total || 0).toLocaleString()}
                        </p>
                      </div>
                      <div className="flex gap-2">
                        <Button
                          ...
                          variant="outline"
                          onClick={() => {
                            const id = estimate.estimate_id || estimate.cost_estimate_id || estimate._id;
                            if (id) {
                              navigate(`/cost-estimates/${id}`);
                            } else {
                              toast.error('No se puede abrir esta estimación - ID no encontrado');
                            }
                          }}
                        >
                        ...
                        <Button
                          ...
                          onClick={() => {
                            setEstimateToAssign(estimate);
                            setAssignForm({ company_id: selectedCompany.company_id, sponsor_id: '' });
                            setAssignDialogOpen(true);
                          }}
                        >
                          <UserPlus className="w-4 h-4 mr-1" /> Asignar Sponsor
                        </Button>
                      </div>
                    </div>
                  ))}
          ...
          {/* Assign/Move Estimate Dialog for orphaned estimates */}
          <Dialog open={assignDialogOpen} onOpenChange={setAssignDialogOpen}>
            <DialogContent className="max-w-md">
              <DialogHeader>
                <DialogTitle>Asignar o Mover Estimación</DialogTitle>
              </DialogHeader>
              <div className="space-y-4">
                ...
                {/* Sponsor selector - based on selected company */}
                {assignForm.company_id && (() => {
                  const selectedComp = companies.find(c => c.company_id === assignForm.company_id);
                  const hasSponsors = selectedComp?.sponsors?.length > 0;
                  ...
                  return hasSponsors ? (
                    <div className="space-y-2">
                      ...
                      >
                        ...
                        <SelectContent>
                          {selectedComp.sponsors.map(sponsor => (
                    ...
                    <div className="p-4 bg-amber-50 border border-amber-200 rounded-lg">
                      <p className="text-sm text-amber-700 mb-3">
                        ...
                        Esta compañía no tiene sponsors. Debes crear uno primero.
                      </p>
                      <Button
                        size="sm"
                        ...
                        onClick={() => {
                          ...
                          setNewSponsorOpen(true);
                        }}
                      >
                        <UserPlus className="w-4 h-4 mr-1" /> Crear Sponsor para {selectedComp?.name}
                      </Button>
                    </div>
                  );
                })()}
                ...
                <div className="flex gap-2 pt-4">
                  ...
                  <Button 
                    ...
                    disabled={!assignForm.company_id || !assignForm.sponsor_id}
                  >
                    {assignForm.company_id === selectedCompany?.company_id ? 'Asignar Sponsor' : 'Mover a Compañía'}
                  </Button>
                </div>
              </div>
            </DialogContent>
        ...
        <Dialog open={assignDialogOpen} onOpenChange={setAssignDialogOpen}>
          <DialogContent className="max-w-md">
            ...
            <div className="space-y-4">
              ...
              {assignForm.company_id && (
                <div className="space-y-2">
                  <Label>Sponsor {getSponsorsForAssign().length === 0 ? '*' : '(opcional)'}</Label>
                  {getSponsorsForAssign().length > 0 ? (
                    <Select 
                      value={assignForm.sponsor_id} 
                      onValueChange={(value) => setAssignForm({...assignForm, sponsor_id: value})}
                    >
                    ...
                    <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          setAssignDialogOpen(false);
                          setSelectedCompany(companies.find(c => c.company_id === assignForm.company_id));
                          setNewSponsorOpen(true);
                        }}
                      >
                        <UserPlus className="w-4 h-4 mr-1" /> Crear Sponsor para esta Compañía
                      </Button>
                    </div>
                  )}
                </div>
              ...
              <div className="flex gap-2 pt-4">
                ...
                <Button 
                  className="flex-1 bg-orange-500 hover:bg-orange-600"
                  onClick={handleAssignEstimate}
                  disabled={!assignForm.company_id || (getSponsorsForAssign().length > 0 && !assignForm.sponsor_id)}
                >
                  Asignar
                </Button>
              </div>
