...
const HumanResources = () => {
  ...
  const loadDocuments = async (employeeId) => {
    try {
      const response = await api.get(`/api/employees/${employeeId}/documents`, { withCredentials: true });
  ...
  const handleSaveProfile = async () => {
    setSaving(true);
    try {
      await api.put(`/api/employees/${selectedEmployee.user_id}/profile`, {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      await api.post(`/api/employees/${selectedEmployee.user_id}/documents?document_type=${uploadForm.document_type}`, formData, { withCredentials: true, headers: { 'Content-Type': 'multipart/form-data' } });
  ...
  const handleDelete = async (docId) => {
    ...
    try {
      await api.delete(`/api/employees/${selectedEmployee.user_id}/documents/${docId}`, { withCredentials: true });
