...
import { Building2, Plus, Mail, Phone, Search, ChevronRight, FileText, Users } from 'lucide-react';
...
const Clients = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.post('/client-profiles', form);
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-slate-500 text-sm">Gestiona los perfiles de tus clientes y sus estimados</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(client => (
            <Card 
              ...
              onClick={() => navigate(`/client-profiles/${client.profile_id}`)}
            >
        ...
        {filtered.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-sm text-slate-400 mt-1">Crea un cliente o crea un estimado para agregar automáticamente</p>
          </div>
        )}
      </div>
    </Layout>
